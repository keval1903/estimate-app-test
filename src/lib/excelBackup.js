export async function generateExcelWorkbook(supabase, activePlatform) {
  try {
    ...
    for (const plat of platformsToBackup) {
      ...
      XLSX.utils.book_append_sheet(wb, wsClients, `${platPrefix}Client Balances`.substring(0, 31))
      XLSX.utils.book_append_sheet(wb, wsEstimates, `${platPrefix}Estimates & Quotes`.substring(0, 31))
      XLSX.utils.book_append_sheet(wb, wsPayments, `${platPrefix}Payment History`.substring(0, 31))
    }
...
export async function downloadExcelBackup(supabase, activePlatform) {
  ...
  XLSX.writeFile(wb, `Ledger_Backup_${activePlatform.toUpperCase()}_${dateStr}.xlsx`)
}
